...
import rdf from './ldf/util/RdfUtil'
...
function FederatedFragmentsClient (startFragments, options) {
  ...
  if (!_.isArray(startFragments)) {
    return new FragmentsClient(startFragments)
  }
  if (startFragments.length === 1) {
    return new FragmentsClient(startFragments[0])
  }
...
function CompoundFragment (fragments, options) {
  ...
  let fragmentsPending = fragments.length
  let metadataPending = fragments.length
  let errorThreshold = options.errorThreshold || 0
  let combinedMetadata = this._metadata = { totalTriples: 0 }
